(function () {
    ...
    function run(stepsPerFrame) {
        ...
        runTimer = setInterval(() => {
            for (let i = 0; i < Math.max(1, stepsPerFrame); i += 1) {
    ...
    function getState() {
        return {
            ...
            state: machine.state,
            error: machine.error,
            breakpoints: new Set(Array.from(machine.breakpoints)),
            stepCount: machine.stepCount
        };
    }

    window.VEIZACSimulator = {
        machine,
        WORD_MASK,
        mask40,
        extractLeft,
